...
describe('classifyMetadataKey — §3', () => {
  ...
  it('pins "alias" as unknown — the stashed display name (agent-manager-db.ts:6908)', () => {
    // The rename path moves the OLD name here. Counted BY FORM 2026-07-26 —
    // 28 reads of the stored key (15 JS-form + 13 SQL-form, which no `.alias`
    // regex can see) across 6 files, plus 15 reads of the flattened `.alias`
    // API field across 6 files including two .tsx TUI components — 43 across
    // 12 files. Excludes `parsed.alias` (a parsed agent reference, not this
    // key) and the homonyms normalizeAlias / resolveModelAlias / agentAlias.
