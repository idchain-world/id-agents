...
Origin submission attribution is status-scale metadata with no payload: it survives compaction and is removed when its message becomes a receipt.
...
Completion does not create a local agent callback, query reply, or news item. Conversations belong to an origin **team**, not to a sending agent; that agent may have stopped, and choosing it would make an equally authorized teammate miss the signal. Per-message sender attribution explains who submitted a turn but grants no collection authority. Copying result payloads into news would also create a second retention domain and source of truth. V1 therefore keeps list-plus-collect polling as the single delivery contract. A future status-only team-inbox nudge may be added if measured polling cost justifies it, but it is not part of V1.
...
A team can already name a team on another machine, because a contact pins a remote `(nodeId, teamId)` and acceptance is durable and idempotent. What is missing is the transport. Today a contact pinned to a foreign node stops at `peer_route_unconfigured`, nothing maps a node identity to an address, the acceptance path is exposed on no network, and continue and collect read the destination's conversation rows out of the same database, which does not exist when the destination is another machine. Phase E adds a transport around the existing message contract. It adds no second acceptance contract, no second message schema, and no second identity model.

**Peer routes.** A peer route is node-global operational configuration mapping one expected remote `nodeId` to the one address where that node can currently be reached. It is not team-scoped, because a manager has one node identity and one listener for all of its teams. It stores the expected `nodeId` as its immutable key, one absolute federation base address, whether it is enabled, and inspection timestamps. V1 has one active address per node; priority lists, discovery, failover pools, health history, credentials, and team grants are not part of it. Changing an address never rewrites a contact, conversation, message, receipt, or envelope, because those pin identity and the route only answers how to reach that identity now. Changing the expected identity is not an address edit: the operator removes the old route, creates one for the new node, and explicitly changes any contacts that should point to it. A route naming the local `nodeId` is refused, and a pin equal to the local node never consults routes at all, so same-manager delivery cannot be federated even by misconfiguration. Routes are written only through the loopback operator surface; a federation request or response can never create, update, disable, or suggest one, and redirects and reply URLs are not route discovery. A stale address is not a missing route: connection failure and timeout are transport failures, and the manager never deletes or rewrites a route because one request failed.

**Refusing a substituted peer.** Every federation response, successful or not, identifies the responding node with its durable `nodeId` and protocol version, and the origin compares that value with the route key before trusting any result. A different node returns `peer_node_mismatch`: the origin discards the response's protocol outcome, preserves any already-durable outbound envelope, and raises a diagnostic naming the expected node, the actual node, and the configured address. It does not update the route, retarget the contact, mark the message accepted, or try another address. A missing or malformed responder identity is `peer_response_invalid`, never permission to infer identity from DNS, the socket address, the requested team, or a previous response. This catches stale and accidentally substituted routes. It is not authentication and does not catch a peer that lies about its `nodeId`.

**Topology portability.** Topology is not identity. The same two logical nodes must move between private topologies without changing application code or durable messaging state, and such a move changes only each node's configured bind address and the local peer-route rows. It must not require a code change, rebuild, node or team re-registration, new `nodeId`, contact mutation, or any rewrite of conversations, messages, origin submissions, deduplication records, results, or receipts. Two settled decisions carry this: node identity is intrinsic, and contacts pin identity rather than location. No route, origin-state, acceptance, deduplication, participant-binding, continuation, collection, or receipt key may contain or derive identity from a hostname, IP, interface, container name, port, network, or base address. An address appears only in bind configuration and in the mutable value of a route. Every outbound attempt resolves the route late, by the conversation's pinned destination `nodeId`, so a route update applies to a new send, an identical resubmission, a continuation, collection, and descriptor reads without touching their durable identity. An implementation that snapshots an endpoint into a contact, conversation, message, receipt, or envelope is non-conforming even if its first delivery succeeds. Moving the existing node, database included, is the supported path, because `manager_identity` travels with that database. Standing up a fresh database at a new address is not a re-addressing: it is a different logical node with a new `nodeId`, its contacts must be created deliberately, and contacts pinned to the old identity keep failing as `target_identity_missing` rather than being silently retargeted.

**Durable origin state.** Same-manager delivery can read the destination's rows in one database and federation cannot, so the origin keeps its own record of every outbound conversation and submission: the complete envelope needed for an identical resubmission including its original protocol version and `firstSubmittedAt`, the origin and destination pins, the destination variant and any pinned recipient facts, the conversation and message IDs with position and predecessor, the origin-local sender attribution, whether the attempt has not happened or its outcome is unknown, confirmed accepted, or confirmed rejected, and the last attempt time and diagnostic. The endpoint is part of none of it, and is not persisted on any conversation, message, submission, deduplication, result, or receipt row; attempt diagnostics may name an address for operator inspection, but that text is non-authoritative and can never be reused as a route. This record is not a second authority for destination state: the destination remains authoritative for every status, and collection always asks it. A continuation is never minted past an earlier submission whose acceptance is still unknown; the origin first collects or resubmits that envelope and advances only once acceptance is confirmed, so a lost response cannot turn the next action into a new position or a fork.

**Listener separation.** The management API and the federation listener are different trust surfaces with different configuration. Management stays bound to `127.0.0.1` with all existing operator behavior, and federation configuration cannot widen it. The federation listener is a separate server with its own bind address and port, exposing only submission including continuations and identical resubmissions, collection by conversation and message ID, descriptor and roster reads by immutable destination team ID, and the responder identity on every response. It exposes no management configuration, contact or route mutation, task, schedule, wallet, library, artifact, worker, WebSocket, admin, or local conversation-index operation, and it interprets no local `X-Id-Admin`, `X-Id-Team`, or `X-Id-Agent` header as authority. It is disabled by default: with no bind configured no socket is opened, an inbound peer gets only a connection failure, and the management API is never a fallback federation endpoint. Enabling it requires an explicit address and port; startup refuses an incomplete or invalid bind, and refuses the wildcard `0.0.0.0` or `::` unless a separate explicit exposure option is set, because the wildcard silently exposes the port to every attached network and a warning is not a gate. The bound address is the trust boundary in operational terms, so the process reports its exact federation bind at startup. Listener and route settings are independent: a node may accept inbound federation with no outbound route, and an origin may hold a route while its own listener is disabled, because pull-only results need a route in one direction only.

**The wire.** The federation transport reuses the existing envelope and collection result. Transport supplies the asserted origin node and team to the existing federation acceptance context, the envelope must agree with it, and the receiver rejects a request claiming its own node, a source-context mismatch, a foreign destination node, an unsupported major, or any existing pre-accept violation before durable acceptance. Every response is bounded and carries the responder's `nodeId` and protocol version outside the operation result, and the origin validates those before interpreting anything. A remote send resolves the contact and pin, resolves the current route by destination `nodeId`, durably records the envelope, submits, and treats `202` as success only after the identity check; the receiver returns `202` only after its acceptance transaction commits. A lost response, timeout, reset, invalid response, or peer 5xx is an unknown outcome: the origin never infers failure, never mints a replacement message or position, and either collects by the durable IDs or resubmits the identical envelope, which recovers a committed status without a second acceptance. The 30-day resubmission horizon runs from the original `firstSubmittedAt`, and no restart, upgrade, timeout, or address change resets it. A peer that accepted but has not finished is not a transport failure, and no local timeout turns that work into `failed`.

**Remote reads.** Collection stays pull-only, origin-only, idempotent, and non-consuming. Transport supplies the asserted origin node and team, the request supplies the pinned destination team, and the receiver authorizes by exact equality with the durable participants, so a caller cannot substitute participants through request parameters. Unknown conversation, unknown message, deleted participant, and participant mismatch all return the same `conversation_not_found` without revealing which check failed, and inbound policy never affects collection of already-accepted work. Descriptors are read by immutable team ID; contact aliases never cross the wire because they are origin-local vocabulary, and the origin may decorate its own response with the current alias after validating the responder. The published projection is exactly the settled descriptor and roster projection, exposing no alias, route, URL, port, path, task, hierarchy, lead hint, wallet, sender attribution, or conversation data. Receiver-side read limiting is keyed by the asserted source node, because different peers must not share one local bucket.

**Failure taxonomy.** `peer_route_unconfigured`, `peer_route_invalid`, `peer_unreachable`, `peer_timeout`, `peer_node_mismatch`, `peer_response_invalid`, and `resubmission_horizon_exceeded` are origin-local results and diagnostics. They never cross the federation wire and never enlarge the receiver's fixed pre-accept code set. Transport failures are retryable as the same durable operation and never permit changing an envelope under the same message ID; `peer_node_mismatch` is not retryable until an operator repairs the route. A timeout, 5xx, or malformed response is never converted into `failed`, because the origin cannot prove the receiver did not commit. Repeated transport failures, route mismatch, invalid listener configuration, and submissions whose acceptance stays unknown must remain operator-visible rather than collapsing into a generic `target_identity_missing` or `conversation_not_found`.

**Backward compatibility.** A node with no peer routes and no federation bind behaves exactly as before: management on `127.0.0.1`, no additional listener, local contacts through the same in-process acceptance service, foreign contacts still returning `peer_route_unconfigured`, and every existing envelope, comparison identity, version rule, status machine, retention rule, and pre-accept code unchanged. Peer-route and outbound persistence are additive, retarget no contact, infer no route from a contact, create no default route, and enable no listener.

**Commit 14 — Peer routes and durable outbound state.** Persist the node-global route requirements and the origin-side conversation and envelope facts in both databases. Add loopback operator flows for route inspection, create, address change, enable and disable, and removal. Declare the transport seam with an unconfigured default so ordering is observable, and keep local-node delivery in process. No federation socket and no network client exist in this commit. Route absence preserves `peer_route_unconfigured`, route changes rewrite no contact or envelope, and outbound state is committed before any transport can observe an attempt, survives restart, preserves the original protocol version and horizon, serializes continuations behind unknown submissions, and supports the origin-team index without a destination row in the same database.
*Gate:* database parity and restart tests pass; route keys are node-global and cannot name the local node; invalid routes are refused; missing and disabled routes preserve today's result; an address change leaves every identity pin, conversation, message, origin submission, deduplication record, result, receipt, and envelope byte-for-byte equivalent; no endpoint column or address-derived key exists on those records; the existing same-manager suites still pass; no process opens a federation listener.

**Commit 15 — Federation contract and two-manager fault tests.** Define the bounded request and response contracts and connect them to the existing acceptance, collection, and descriptor services through a federation app on an ephemeral loopback port, with the outbound client behind the commit-14 seam. Use two managers with distinct databases, node IDs, team IDs, work roots, and management ports, and enable no configured federation bind. The response identifies its node before the origin trusts an outcome, remote collection derives authorization from the asserted origin and the durable participant binding, descriptor reads address the immutable remote team ID and never publish the origin alias, and lost responses preserve the exact envelope without advancing the stream.
*Gate:* a first send, continuation, processing result, repeated collection, and roster read cross the two-manager boundary; a lost `202` followed by resubmission creates one destination message and returns `deduplicated: true`; a lost continuation response is recovered before the next position is allocated; unauthorized and unknown collection are identical `conversation_not_found`; policy-closed roster reads still work; a route aimed at the wrong manager returns `peer_node_mismatch` and changes no pin; the destination makes zero outbound connection attempts.

**Commit 16 — Configured listener, outbound client, and private-network operations.** Add the separately configured federation listener and outbound client on the frozen commit-15 contracts. Keep management loopback-only, default federation to disabled, refuse incomplete binds, and refuse wildcard binds without the separate explicit override. Expose only the federation operations above, and add bounded deadlines, response sizes, concurrency, diagnostics, and the failure taxonomy without changing accepted-message states. Document the trusted-network requirement, the exact active bind, peer-route and contact setup, inbound policy, team lead, and failure diagnostics. Authentication, encryption, and multi-tenancy remain explicitly absent.
*Gate:* two node containers run with distinct durable databases on one private Docker network; each management API is loopback-only; only the receiver exposes its federation listener; only the sender has a route, pinned to the receiver's `nodeId`; a team on one sends to a team on the other, the receiver processes exactly once, and the sender repeatedly collects the durable result; the same test survives a lost acceptance response and a destination restart; the same databases and build artifacts then move to another private topology by changing only bind addresses and route values, and completed, in-flight, and unknown-outcome conversations survive with identical node IDs, pins, messages, and receipts and continue without re-registration; a fresh-database control receives a new node ID and inherits no contact or conversation; listener absence exposes nothing; wildcard startup refuses without the override; a substituted-node route fails closed; same-manager delivery still passes; and the destination's outbound-network spy records zero attempts.
...
- The wire `nodeId` is a non-secret value taken at face value on the configured federation listener for inbound authorization. An origin separately checks the `nodeId` a route returns against that route's key before trusting a response, which detects accidental substitution and is not authentication. Contacts pin remote identity and node-global peer routes store only the local operational address used to reach a pinned node; a peer cannot create or update that address, and no route participates in message identity or authorization.
