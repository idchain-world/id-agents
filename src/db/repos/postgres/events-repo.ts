...
export class PgEventsRepo implements EventsRepository {
  ...
  async query(opts: {
    ...
    if (opts.topics && opts.topics.length > 0) {
      sql += ` AND topic = ANY($${idx++}::text[])`;
      params.push(opts.topics);
    }

    sql += ` ORDER BY seq ASC LIMIT $${idx}`;
    params.push(limit);
  ...
  async earliestSeq(teamId: string): Promise<number | null> {
    const { rows } = await this.db.query<{ seq: string | number | null }>(
      `SELECT MIN(seq) AS seq FROM event_log WHERE team_id = $1`,
      [teamId],
    );
